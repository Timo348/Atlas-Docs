...
export function PageShareDialog({ pageId, pageTitle, allowEdit = true, onClose }: { pageId: string; pageTitle: string; allowEdit?: boolean; onClose: () => void }) {
  return <ShareDialog targetId={pageId} targetTitle={pageTitle} targetKind="page" allowEdit={allowEdit} onClose={onClose} />;
}

export function FolderShareDialog({ folderId, folderName, onClose }: { folderId: string; folderName: string; onClose: () => void }) {
  return <ShareDialog targetId={folderId} targetTitle={folderName} targetKind="folder" allowEdit onClose={onClose} />;
}

function ShareDialog({ targetId, targetTitle, targetKind, allowEdit, onClose }: {
  targetId: string;
  targetTitle: string;
  ...
  const [label, setLabel] = useState(targetTitle.slice(0, 80));
  const [permission, setPermission] = useState<"VIEW" | "EDIT">("VIEW");
  const [expiry, setExpiry] = useState("30");
  const [createdUrl, setCreatedUrl] = useState("");
  const [copied, setCopied] = useState(false);
  useDialogEscape(onClose, busy, true);
  const endpoint = targetKind === "folder" ? `/api/folders/${targetId}/shares` : `/api/pages/${targetId}/shares`;
  const targetLabel = targetKind === "folder" ? text("folder", "Ordner") : text("page", "Seite");
    ...
    void fetch(endpoint, { cache: "no-store" })
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(apiErrorMessage(result, text, {
          en: "Share links could not be loaded.", de: "Freigabelinks konnten nicht geladen werden.",
        }));
        if (active) setShares(result as PageShareRow[]);
      })
      .catch((reason) => active && setError(reason instanceof Error ? reason.message : text("Share links could not be loaded.", "Freigabelinks konnten nicht geladen werden.")))
  ...
  async function createShare() {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(apiErrorMessage(result, text, {
        en: "The share link could not be created.", de: "Der Freigabelink konnte nicht erstellt werden.",
      }));
      ...
      setError(reason instanceof Error ? reason.message : text("The share link could not be created.", "Der Freigabelink konnte nicht erstellt werden."));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function updatePermission(share: PageShareRow, nextPermission: "VIEW" | "EDIT") {
    ...
    setError("");
    try {
      const response = await fetch(`${endpoint}/${share.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function revokeShare(share: PageShareRow) {
    ...
    setError("");
    try {
      const response = await fetch(`${endpoint}/${share.id}`, { method: "DELETE" });
      if (!response.ok) {
        const result = await response.json();
        throw new Error(apiErrorMessage(result, text, {
          en: "The share link could not be revoked.", de: "Der Freigabelink konnte nicht widerrufen werden.",
        }));
      }
      ...
      setError(reason instanceof Error ? reason.message : text("The share link could not be revoked.", "Der Freigabelink konnte nicht widerrufen werden."));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && !busy && onClose()}>
      <section className="page-share-dialog" role="dialog" aria-modal="true" aria-label={text(`Share ${targetLabel}`, `${targetLabel} freigeben`)}>
        <header className="dialog-header">
          <div><span className="dialog-kicker">Atlas</span><h2>{targetKind === "folder" ? text("Share this folder", "Diesen Ordner freigeben") : text("Share this page", "Diese Seite freigeben")}</h2></div>
        ...
        <div className="page-share-intro">
          <Shield size={18} />
          <p>{text(
            targetKind === "folder" ? "Anyone holding a link gets access to this folder, its subfolders, and their files. Links are stored as hashes and can be changed or revoked here." : "Anyone holding a link gets access to this page only. Links are stored as hashes and can be changed or revoked here.",
            targetKind === "folder" ? "Jeder mit dem Link erhält Zugriff auf diesen Ordner, seine Unterordner und deren Dateien. Links werden gehasht gespeichert und können hier geändert oder widerrufen werden." : "Jeder mit dem Link erhält nur Zugriff auf diese Seite. Links werden gehasht gespeichert und können hier geändert oder widerrufen werden.",
          )}</p>
        </div>
        <div className="page-share-body">
          ...
          <section className="page-share-create">
            ...
            <div className="page-share-create-grid">
              ...
              <label><span>{text("Access", "Zugriff")}</span><select value={permission} onChange={(event) => setPermission(event.target.value as "VIEW" | "EDIT")}><option value="VIEW">{text("View only", "Nur lesen")}</option>{allowEdit && <option value="EDIT">{text("Edit content", "Inhalt bearbeiten")}</option>}</select></label>
            ...
            {createdUrl && (
              <div className="page-share-created">
                ...
                <input readOnly value={createdUrl} onFocus={(event) => event.currentTarget.select()} aria-label={text("Created share link", "Erstellter Freigabelink")} />
          ...
          <section className="page-share-list-section">
            ...
            {!loading && !activeShares.length && <div className="page-share-empty">{text("No active share link yet.", "Noch kein aktiver Freigabelink.")}</div>}
            <div className="page-share-list">
              {activeShares.map((share) => (
                <article className="page-share-row" key={share.id}>
                  ...
                  <select disabled={busy || !allowEdit} value={allowEdit ? share.permission : "VIEW"} aria-label={text(`Access for ${share.label}`, `Zugriff für ${share.label}`)} onChange={(event) => void updatePermission(share, event.target.value as "VIEW" | "EDIT")}><option value="VIEW">{text("View", "Lesen")}</option>{allowEdit && <option value="EDIT">{text("Edit", "Bearbeiten")}</option>}</select>
